...
class PackagingAssetTests(unittest.TestCase):
    ...
    def test_dbus_and_systemd_service_files_reference_the_frozen_identity(self) -> None:
        ...
        self.assertTrue(systemd_service_path.exists())
        self.assertIn("ExecStart=", systemd_service)
        self.assertIn("kdictate-daemon", dbus_service)
        self.assertNotIn("--profile", dbus_service)
        self.assertIn("kdictate-daemon", systemd_service)
        self.assertNotIn("--profile", systemd_service)
