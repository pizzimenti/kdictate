...
- `--noise-floor-margin`: how far above the measured ambient noise floor a block must sit to count as speech. **Defaults to `0` — the adaptive gate is off** and `--energy-threshold` is used alone. Set it (try `1.6`) to opt in: the gate becomes `max(--energy-threshold, noise_floor * margin)`, with no upper clamp. The noise floor is measured and logged either way, so tune against the `recording ended:` line — the margin only helps if `noise_floor` sits well below your speaking level. On hardware where a push-to-talk session is short and mostly speech, the estimate tracks the speaker rather than the room and the margin is best left at `0`.
- `--mic-min-volume-percent`: capture-volume **floor** (default `50`). On activation the daemon raises the default source to this level only if it is currently *below* it, and never lowers it — above the floor your gain is your own. `0` leaves the microphone untouched entirely. If `recording ended:` reports `peak_rms` at or near `32768`, the microphone is clipping and no VAD setting can compensate: turn the input level down in system settings.
